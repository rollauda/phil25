---
sidebar_position: 1
id: accueil
---
# Leçon 4 : Suis-je celle ou celui que je pense être ?

- **Plan de la leçon** : 
  - [Introduction : Le problème de l'identité personnelle](/docs/L4/0/intro.md)
  - [1. Que suis-je ?](/docs/L4/1/1.md)
  - [2. Qui suis-je ?](/docs/L4/2/2.md)
  - [3. Je est-il un autre ?](/docs/L4/3/3.md)
- **Perspectives**  : 1. L'existence et la culture ; 3. La connaissance 
- **Notions principales** : `CONSCIENCE`, `INCONSCIENT`  
- *Notions complémentaires* : *`Raison`*, *`Vérité`*, *`Bonheur`*   
- *Repères conceptuels* : *[Identité/Égalité/Différence](/manuel/Repères/identite)*
- **Méthode** : L'explication de texte
- **Auteurs étudiés** : Plutarque, R. Descartes, D. Hume, B. Pascal, S. Freud   
- **Travaux** :
  - **Reprendre dans un carnet les définitions du cours à retenir**. 
  - **Écrire une courte synthèse de la leçon leçon lorsqu'elle est terminée** (vous pourrez être interrogés au début de la leçon suivante) : Qu’est-ce que j’ai retenu ? *(Je note les idées-clés que je retiens de la leçon, les thèses des auteurs lus ou les questions qu’ils posent)* 
  - **Évaluation** : Plan détaillé d'explication de texte en classe (2h).

